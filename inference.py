...
def inference(args):
    ...
    if args.model_name == "llama":
        ...
        ModelCls = SKDLlamaForCausalLM
    elif args.model_name == "mistral":
        base_model = "mistralai/Mistral-7B-Instruct-v0.1"
        ModelCls = SKDMistralForCausalLM
    else:
        raise ValueError("args.model_name must be llama or mistral")

    # Decide what to load:
    # - run_option == "inference": load exported student folder (./llama-dskd) which contains model.safetensors + config.json + tokenizer
    # - otherwise: load base model
    if args.run_option == "inference":
        load_path = args.load_student_ckpt_path   
    else:
    ...
    model = ModelCls.from_pretrained(
        ...
        token=HF_TOKEN if args.model_name == "llama" else None,
        trust_remote_code=True,
    )

    print(model)

    # Training args (even for inference your code expects this)
    training_args = {"run_option": args.run_option, "loss_type": args.loss_type}
    model.set_training_args(training_args)

    # Move to GPU & set rank
    ...
    if "mmlu" in args.task_name and (args.run_option in {"original", "inference","teacher_replace"}):
